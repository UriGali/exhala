...
export default function ProfilePage() {
  ...
  const handleActivatePush = async () => {
    ...
    try {
      ...
      if (result.success) {
        setPushFeedback('¡Notificaciones activas! Recibirás alertas SOS de tus amigos.')
      } else {
          ...
          >
            <div className="flex items-center justify-between pb-2 border-b border-[rgba(232,183,94,0.08)]">
              <span className="text-[11px] font-semibold uppercase tracking-wider text-[#7C9481]">
                Avisos de Emergencia SOS
              </span>
              <Bell className="w-3.5 h-3.5 text-[#E8B75E]" />
            </div>

            <p className="text-[12px] text-[#A9BBA4] leading-relaxed">
              Recibe notificaciones en este dispositivo cuando tus compañeros activen el botón SOS o rieguen tu jardín.
            </p>

            {pushFeedback && (
              <p className="text-xs px-3 py-2 rounded-xl bg-[rgba(232,183,94,0.1)] text-[#E8B75E] border border-[rgba(232,183,94,0.2)]">
                {pushFeedback}
              </p>
            )}

            {pushPermission === 'granted' ? (
              <div className="flex items-center justify-between bg-emerald-500/10 border border-emerald-500/30 rounded-xl px-3.5 py-2.5">
                <div className="flex items-center gap-2 text-xs text-emerald-300 font-medium">
                  <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
                  <span>Avisos web activos</span>
                </div>
                <button
                  type="button"
                  onClick={handleActivatePush}
                  disabled={isActivatingPush}
                  className="text-[11px] text-[#E8B75E] underline hover:text-[#F1EEE2] cursor-pointer"
                >
                  Actualizar
                </button>
              </div>
              ...
              <button
                ...
                onClick={handleActivatePush}
                disabled={isActivatingPush}
                className="w-full py-2.5 bg-[rgba(232,183,94,0.15)] hover:bg-[rgba(232,183,94,0.25)] border border-[rgba(232,183,94,0.3)] text-[#E8B75E] text-xs font-semibold rounded-xl flex items-center justify-center gap-2 transition-all cursor-pointer"
              >
                {isActivatingPush ? (
                  <>
                  ...
                  <>
                    ...
                    <span>Activar avisos en este móvil</span>
                  </>
                )}
              </button>
            )}
